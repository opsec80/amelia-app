...
async function initTasksFile() {
    try {
        ...
        console.warn('Initializing tasks.json:', err.message);
        try {
            ...
            await fs.writeFile(TASKS_FILE, JSON.stringify([
                ...
                {"id":"14","name":"Vacuum","size":"medium","month":"2025-07","recurring":"none","completed":false,"dueDate":"2025-07-25"},
...
async function calculateTaskValues() {
    try {
        ...
        tasks.forEach(task => {
            task.value = unitValue * (weights[task.size] || 1);
        });
