...
function displayTasks(tasks) {
    ...
    tasks.forEach(task => {
        ...
        li.innerHTML = `
            <span>${task.name} ($${task.value.toFixed(2)}) ${task.dueDate ? 'Due: ' + task.dueDate : ''}</span>
...
async function requestPayment() {
    try {
        ...
        const body = `Tasks Completed:\n${tasks.filter(t => t.completed).map(t => `- ${t.name}: $${t.value.toFixed(2)}`).join('\n')}\nTotal: $${earned.toFixed(2)}\nBonus: $${bonus}`;
...
async function loadAdminTasks() {
    try {
        ...
        tasks.forEach(task => {
            ...
            li.innerHTML = `
                <span>${task.name} ($${task.value.toFixed(2)}, ${task.size}, ${task.month}, ${task.recurring}, Due: ${task.dueDate || 'N/A'})</span>
...
function displaySubmissions(tasks) {
    ...
    submissionList.innerHTML = tasks.filter(t => t.completed).map(t => `
        <li>${t.name} by ${t.userName || 'Unknown'} ($${t.value.toFixed(2)}, ${new Date(t.completedDate).toLocaleDateString()})
...
function downloadReport() {
    fetch(`${API_URL}/tasks`)
        ...
        .then(tasks => {
            ...
            tasks.forEach(t => {
                csv.push(`${t.name},${t.size},${t.value.toFixed(2)},${t.completed ? 'Yes' : 'No'},${t.completedDate || ''},${t.picture || ''},${t.dueDate || ''}`);
            });
